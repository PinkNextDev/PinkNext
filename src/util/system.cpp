...
const char * const BITCOIN_CONF_FILENAME = "pinkcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "pinkcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Pinkcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Pinkcoin
    // Mac: ~/Library/Application Support/Pinkcoin
    // Unix: ~/.pinkcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pinkcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pinkcoin";
#else
    // Unix
    return pathRet / ".pinkcoin";
#endif
#endif
}
